			...
			<tr>
				...
				<c:if test="${board.del != 'y'}">
					<td title="${board.content }">
						...
						<a href="a3_content.jsp?num=${board.num }&pageNum=<%=currentPage %>">${board.subject }</a>
...
<div align="center">
	<% if (startPage > PAGE_PER_BLOCK) { %>
		<button onclick="location.href='a1_list.jsp?pageNum=<%=startPage-1 %>'">[이전]</button>
	<% } %>
	<c:forEach var="i" begin="<%=startPage %>" end="<%=endPage %>">
		<button onclick="location.href='a1_list.jsp?pageNum=${i }'">${i }</button>
	</c:forEach>
	<% if (endPage < totalPage) { %>
		<button onclick="location.href='a1_list.jsp?pageNum=<%=endPage+1 %>'">[다음]</button>
	<% } %>
	<br>
	<button onclick="location.href='a2_writeForm.jsp?num=0&pageNum=1'">글쓰기</button>
</div>
</body>
</html>